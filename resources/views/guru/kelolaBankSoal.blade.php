...
@section('content')
                                ...
                                <tbody>
                                    @foreach (session('import_preview_soals') as $pSoal)
                                        <tr>
                                            ...
                                            <td class="{{ $pSoal['kunci_jawaban'] === 'A' ? 'font-bold text-emerald-800 bg-emerald-50' : '' }}">
                                                {{ $pSoal['opsi_a'] }}
                                            </td>
                                            <td class="{{ $pSoal['kunci_jawaban'] === 'B' ? 'font-bold text-emerald-800 bg-emerald-50' : '' }}">
                                                {{ $pSoal['opsi_b'] }}
                                            </td>
                                            <td class="{{ $pSoal['kunci_jawaban'] === 'C' ? 'font-bold text-emerald-800 bg-emerald-50' : '' }}">
                                                {{ $pSoal['opsi_c'] }}
                                            </td>
                                            <td class="{{ $pSoal['kunci_jawaban'] === 'D' ? 'font-bold text-emerald-800 bg-emerald-50' : '' }}">
                                                {{ $pSoal['opsi_d'] }}
                                            </td>
                ...
                <div class="col-lg-5 mb-4">
                    <div class="card border-0 shadow-sm rounded-2xl bg-white overflow-hidden h-100">
                        ...
                        <div class="card-body p-4 bg-slate-50/40 tab-content" id="tabMenuBankSoalContent">
                            ...
                            <div class="tab-pane fade" id="tab-upload-content" role="tabpanel" aria-labelledby="tab-upload-btn">
                                <form action="{{ route($prefixRoute . '.import.preview') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="kategori_soal_id" value="{{ $selectedCategory->id }}">

                                    <div class="alert alert-purple bg-purple-50 border border-purple-200 rounded-xl p-3 mb-3 text-xs">
                                        <div class="d-flex align-items-center gap-2 mb-1">
                                            <i class="fas fa-info-circle text-purple-700 fa-lg"></i>
                                            <strong class="text-purple-950">Upload Dokumen Soal (PDF / Word / Excel)</strong>
                                        </div>
                                        <p class="text-purple-800 mb-0 leading-relaxed">
                                            Unggah file bank soal dalam format <strong>PDF (.pdf)</strong>, <strong>Word (.doc, .docx)</strong>, atau <strong>Excel (.xlsx, .xls)</strong>.
                                        </p>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label text-xs font-bold text-slate-700 uppercase">Kategori &amp; Mapel Terpilih</label>
                                        <input type="text" class="form-control rounded-xl text-xs font-bold bg-white text-purple-900 border-purple-200"
                                            value="{{ $selectedCategory->deskripsi ?: $selectedCategory->nama_kategori }} ({{ $mapel }} - {{ $jenjang }} Kelas {{ $kelas }})" readonly>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label text-xs font-bold text-slate-700 uppercase d-flex justify-content-between align-items-center">
                                            <span>File Dokumen Soal (PDF / Word / Excel)</span>
                                            <a href="{{ route($prefixRoute . '.template') }}" class="text-purple-700 text-xs font-bold hover:underline">
                                                <i class="fas fa-download mr-1"></i> Format Template
                                            </a>
                                        </label>
                                        <div class="border-2 border-dashed border-purple-300 rounded-2xl p-4 text-center bg-white hover:border-purple-500 transition-colors">
                                            ...
                                            <p class="text-xs text-slate-600 font-medium my-2">Pilih file dokumen PDF / Word / Excel dari komputer Anda</p>
                                            <input type="file" name="file_excel" class="form-control form-control-file text-xs" accept=".pdf,.doc,.docx,.xlsx,.xls,.csv" required>
                                            <small class="text-slate-400 d-block mt-2">Ukuran maksimal file: 5MB</small>
                                        </div>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label text-xs font-bold text-slate-700 uppercase">Instruksi / Catatan Pengerjaan (Opsional)</label>
                                        <textarea name="catatan_dokumen" class="form-control rounded-xl text-xs border-slate-200" rows="2" placeholder="Tuliskan petunjuk pengerjaan atau instruksi soal untuk siswa..."></textarea>
                                    </div>

                                    <div class="pt-3 border-top mt-2 d-flex justify-content-end gap-2">
                                        <a href="{{ route($prefixRoute . '.template') }}" class="btn btn-outline-secondary font-bold rounded-xl px-3 py-2 text-xs">
                            ...
                            <div class="card-body p-2 p-sm-3 bg-slate-50/50">
                                <div class="d-flex flex-column gap-2">
                                    @foreach($guruCatDocFiles as $gDocIdx => $gDocPath)
                                        ...
                                        @if($isPdf)
                                            <div class="modal fade p-0" id="modalGuruPreviewDoc_{{ $gDocIdx }}" tabindex="-1" role="dialog" aria-hidden="true" style="padding-right: 0 !important;">
                                                ...
                                                <div class="modal-content border-0 shadow-lg rounded-2xl overflow-hidden">
                                                    ...
                                                    <form action="{{ route($prefixRoute . '.soal.update', $soalItem->id) }}" method="POST">
                                                            ...
                                                            <div class="row g-3">
                                                                ...
                                                                <div class="col-md-9 mb-3">
                                                                    ...
                                                                    <div class="d-flex flex-wrap gap-2">
                                                                        @foreach (['A', 'B', 'C', 'D'] as $kunci)
                                                                            <div class="form-check form-check-inline m-0">
                                                                                <input class="form-check-input d-none" type="radio" name="kunci_jawaban" id="edit_kunci_{{ $soalItem->id }}_{{ $kunci }}" value="{{ $kunci }}" {{ old('kunci_jawaban', $soalItem->kunci_jawaban) === $kunci ? 'checked' : '' }} required>
                                                                                <label class="btn {{ old('kunci_jawaban', $soalItem->kunci_jawaban) === $kunci ? 'btn-purple text-white shadow-sm' : 'btn-outline-purple' }} text-xs font-bold px-3 py-1.5 rounded-xl cursor-pointer" for="edit_kunci_{{ $soalItem->id }}_{{ $kunci }}">
                                                                                    Opsi {{ $kunci }}
                                                                                </label>
                                                                            </div>
                                                                        @endforeach
                                                        ...
                                                        <div class="modal-footer bg-white py-3 px-4 border-top">
                                                            <button type="button" class="btn btn-secondary font-bold rounded-xl px-4 py-2 text-xs" data-dismiss="modal">Batal</button>
                                                            <button type="submit" class="btn btn-purple font-bold rounded-xl px-4 py-2 text-xs shadow-sm">Simpan Perubahan</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
            ...
            <div class="modal-content border-0 shadow-lg rounded-2xl overflow-hidden">
                <div class="modal-header bg-emerald-800 text-white py-3 px-4">
                    <h5 class="modal-title font-bold text-base d-flex align-items-center" id="modalImportExcelLabel">
                        <i class="fas fa-file-excel text-emerald-300 mr-2"></i> Import Soal dari File Excel
                    </h5>
                    <button type="button" class="close text-white opacity-80 hover:opacity-100" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                ...
                <form action="{{ route($prefixRoute . '.import.preview') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer bg-white py-3 px-4 border-top">
                        <button type="button" class="btn btn-secondary font-bold rounded-xl px-4 py-2 text-xs" data-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-emerald font-bold rounded-xl px-4 py-2 text-xs text-white shadow-sm" style="background-color: #059669; border-color: #059669;">Pratinjau Data Excel</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        function updateLivePreview() {
            var radioKunci = document.querySelector('input[name="kunci_jawaban"]:checked');
            if (radioKunci) {
                var selectedVal = radioKunci.value;
                document.querySelectorAll('label[for^="input_kunci_"]').forEach(function(lbl) {
                    lbl.className = 'btn btn-outline-purple text-xs font-bold px-3 py-1.5 rounded-xl cursor-pointer';
                });
                var activeLbl = document.querySelector('label[for="input_kunci_' + selectedVal + '"]');
                if (activeLbl) {
                    activeLbl.className = 'btn btn-purple text-white shadow-sm text-xs font-bold px-3 py-1.5 rounded-xl cursor-pointer';
                }
            }
        }
    </script>
