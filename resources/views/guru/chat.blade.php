...
@section('title', 'Chat Siswa · Paradise of Math')
...
@section('content')
    <div id="chat-page-scope">
                ...
                <div class="row align-items-center">
                    <div class="col-sm-7">
                        <h1 class="m-0 font-weight-bold text-purple-950 d-flex align-items-center page-title-mobile">
                            <i class="fas fa-comments text-purple-600 mr-2.5"></i> Chat Siswa
                        </h1>
                        <p class="text-sm text-slate-500 mb-0 mt-1">
                            Hubungi langsung siswa yang Anda bimbing.
                        </p>
                    </div>
                    <div class="col-sm-5">
                        <ol class="breadcrumb float-sm-right text-sm bg-transparent p-0 m-0 mt-2 mt-sm-0">
                            <li class="breadcrumb-item"><a href="{{ route('guru.dashboard') }}" class="text-purple-600 font-semibold">Dashboard</a></li>
                            <li class="breadcrumb-item active text-slate-500">Chat Siswa</li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <section class="content pb-4">
            <div class="container-fluid">
                        ...
                        <div class="card border-0 shadow-sm rounded-2xl overflow-hidden h-100 flex flex-col bg-white">
                            <div class="card-header bg-purple-950 text-white py-3">
                                <h5 class="card-title font-weight-bold mb-0 text-sm">
                                    <i class="fas fa-user-graduate mr-2"></i> Siswa Bimbingan
                                </h5>
                            </div>
                            ...
                            <div id="contacts-list-container" class="card-body p-0 overflow-y-auto flex-1">
                                <div class="text-center py-5 text-muted text-xs">
                                    ...
                                    <p class="mb-0">Memuat daftar siswa...</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-8 h-100" id="chat-column">
                        <div class="card border-0 shadow-sm rounded-2xl overflow-hidden h-100 flex flex-col bg-white" id="active-chat-card">
                            ...
                            <div id="chat-welcome-screen" class="h-100 flex flex-col items-center justify-center text-center p-5">
                                ...
                                <h4 class="font-weight-extrabold text-purple-950 mb-2">Chat dengan Siswa</h4>
                                <p class="text-xs text-muted max-w-sm">
                                    Pilih salah satu siswa di panel sebelah kiri untuk mulai mengirimkan pesan.
                                </p>
                            </div>
                            ...
                            <div id="chat-active-screen" class="h-100 flex flex-col hidden">
                                <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between">
                                    <div class="d-flex align-items-center gap-2.5">
                                        ...
                                        <div>
                                            <h6 class="font-weight-bold text-purple-950 mb-0" id="active-contact-name">Siswa</h6>
                                            <small class="text-[10px] text-muted" id="active-contact-sub">Sekolah</small>
                                        </div>
                                    </div>
                                    <span class="badge bg-emerald-100 text-emerald-800 border border-emerald-200 px-2.5 py-1 text-xxs font-bold uppercase rounded-pill">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 d-inline-block mr-1"></span> Siswa Bimbingan
                                    </span>
                                </div>
                ...
                <!-- Empty state ketika belum ada siswa bimbingan -->
                <div id="no-siswa-state" class="card border-0 shadow-sm rounded-2xl text-center py-5 px-4 bg-white hidden">
                    <div class="card-body">
                        ...
                        <h5 class="font-bold text-purple-950 mb-1">Belum Ada Siswa Bimbingan</h5>
                        <p class="text-slate-500 text-sm max-w-md mx-auto mb-0">
                            Anda belum ditugaskan sebagai guru pendamping untuk siswa manapun oleh Admin.
                        </p>
                    </div>
                </div>
    ...
    <script>
        ...
        function loadContacts() {
            fetch('{{ route('guru.chat.contacts') }}')
                .then(res => res.json())
                .then(data => {
                    const listContainer = document.getElementById('contacts-list-container');
                    const noSiswaState = document.getElementById('no-siswa-state');
                    const chatShell = document.querySelector('.chat-shell');
                    if (!listContainer) return;

                    if (data.length === 0) {
                        chatShell.classList.add('hidden');
                        noSiswaState.classList.remove('hidden');
                        return;
                    }
                    chatShell.classList.remove('hidden');
                    noSiswaState.classList.add('hidden');
                    ...
                    data.forEach(c => {
                        ...
                        const unreadBadge = c.unread_count > 0
                            ...
                            : '';
                        const sekolahChip = c.sekolah ? `<span class="badge bg-purple-100 text-purple-800 text-[9px] px-1.5 py-0.5 rounded ml-1 font-bold">${c.sekolah}</span>` : '';

                        html += `
                            <div onclick="selectContact('${c.session_id}', '${c.contact_name.replace(/'/g, "\\'")}', '${(c.sekolah || '').replace(/'/g, "\\'")}')" class="contact-item p-3 d-flex align-items-center justify-content-between ${isActive}">
                                <div class="d-flex align-items-center gap-2.5 overflow-hidden">
                                    ...
                                    <div class="overflow-hidden">
                                        <div class="font-weight-bold text-purple-950 text-xs d-flex align-items-center flex-wrap">${c.contact_name}${sekolahChip}</div>
        ...
        function selectContact(sessionId, name, sekolah) {
            ...
            document.getElementById('active-contact-sub').textContent = sekolah || 'Siswa Bimbingan';
        ...
        function loadMessages() {
            ...
            fetch(`/guru/chat/messages/${currentSessionId}`)
                .then(res => res.json())
                .then(data => {
                    const container = document.getElementById('messages-container');
                    if (!container) return;
                    ...
                    data.forEach(msg => {
                        ...
                        if (!renderedMsgIds.has(msgId)) {
                            ...
                            const isOwn = msg.sender_role === 'guru';
