...
@section('content')
    ...
    <form action="{{ route('siswa.bukti-bayar.submit') }}" method="POST" enctype="multipart/form-data"
        ...
        @csrf
        <input type="hidden" name="total_sesi" value="{{ $totalSesiBulanIni }}">
        <input type="hidden" name="total_harga" value="{{ $totalBiayaBulanIni }}">

        <!-- ══════════════ LEFT COLUMN: PAYMENT METHODS & UPLOAD (7 COLS) ══════════════ -->
        <div class="lg:col-span-7 p-6 sm:p-10 flex flex-col justify-between scrollable-left">
            <div>
                ...
                <p class="text-sm text-slate-500 mb-8">Pilih opsi transfer bank/e-wallet dan unggah bukti transaksi Anda di bawah ini.
                </p>
            ...
            <div
                ...
                <div>
                    ...
                    <div class="mb-4 flex justify-between items-center">
                        ...
                        <span class="text-[11px] font-semibold text-purple-700 bg-purple-50 px-2.5 py-1 rounded-full border border-purple-100">
                            Bulan {{ $currentMonth->translatedFormat('F Y') }}
                        </span>
                    </div>

                    <!-- Title -->
                    <h3 class="text-2xl font-bold text-violet-950 mb-2">Tagihan Pembayaran</h3>
                    <p class="text-slate-500 text-xs mb-6">Total tagihan dihitung dari kemunculan hari bimbingan yang ditentukan Admin pada bulan ini.</p>
                    ...
                    <div class="space-y-3 text-sm text-slate-600 mb-6">
                        <div class="flex justify-between items-start gap-4 pb-2 border-b border-slate-100">
                            <div>
                                ...
                                <small class="text-slate-400 text-xxs block mt-0.5">({{ $siswa->tipe_paket }})</small>
                            </div>
                        ...
                        <!-- Ringkasan Pertemuan per Mapel Bulan Ini -->
                        <div class="pt-1 space-y-2.5">
                            <span class="block font-bold text-slate-700 text-xs uppercase tracking-wider mb-2">
                                <i class="fas fa-calendar-alt text-purple-600 mr-1"></i> Sesi Bimbingan Bulan Ini:
                            </span>

                            @foreach($rincianMapel as $rincian)
                                <div class="bg-purple-50/70 rounded-xl p-3 border border-purple-100">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="font-bold text-purple-900 text-xs">{{ $rincian['nama_mapel'] }}</span>
                                        <span class="text-[11px] font-bold text-purple-800 bg-white px-2 py-0.5 rounded-full border border-purple-200">
                                            {{ $rincian['jumlah_sesi'] }}x Pertemuan
                                        </span>
                                    </div>
                ...
                <div>
                    <button type="submit" id="btnBayar" class="btn-brand font-bold text-md mb-4 border-0">
                        Kirim Bukti Pembayaran <i class="fas fa-arrow-right ml-1.5"></i>
                    </button>
                    ...
                    <div class="p-3 bg-violet-50/50 rounded-2xl border border-violet-100/50 flex items-start gap-2.5">
                        ...
                        <p class="text-[10px] text-violet-950 mb-0 leading-relaxed font-medium">
                            <strong class="text-violet-700">Aktivasi Otomatis:</strong> Setelah bukti transfer berhasil diunggah, Admin akan langsung memverifikasi transaksi Anda.
                        </p>
                    </div>
                </div>
            </div>
